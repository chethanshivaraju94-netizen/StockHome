...
def load_sector_master_v2():
    ...
    for repo in repos:
        for branch in branches:
            url = f"https://raw.githubusercontent.com/{repo}/{branch}/nse_sector_master.json"
...
def apply_sector_industry_mapping(df):
    """
    Maps each ticker to its official NSE Sector and Industry from nse_sector_master.json.
    If not found in the master dictionary, falls back to map_to_indian_classification().
    """
    if df.empty:
        return df
    ...
    for _, row in df.iterrows():
        # Get bare symbol (e.g., 'NSE:TCS' -> 'TCS')
        ticker = str(row.get("name", "")).split(":")[-1].strip().upper()

        if ticker in master_dict:
            entry = master_dict[ticker]
            mapped_sectors.append(entry.get("Sector", "Unclassified"))
            mapped_industries.append(entry.get("Industry", "Unclassified"))
        else:
            # Fallback using TradingView's raw category strings
            tv_sec = str(row.get("sector", ""))
            tv_ind = str(row.get("industry", ""))
            fb_sec, fb_ind = map_to_indian_classification(tv_ind, tv_sec)
            mapped_sectors.append(fb_sec)
            mapped_industries.append(fb_ind)
...
def fetch_screener_data(exchanges, min_mcap, vol_period_days, ma_columns_to_fetch, limit_rows):
    ...
    select_cols = (
        ["name", "close", "change", "high", "low", "open", "volume", "market_cap_basic",
         ...
         "ADR", "price_52_week_high", "price_52_week_low", "High.1M", 
         "High.3M", "High.6M", "High.All", "exchange", "type",
