...
from modules.ai_analyst import run_gemini_fundamental_analysis, show_fundamental_modal
from modules.chart_studio import render_kline_chart
from modules.config import (
    map_to_indian_classification,
    parse_table_selection_multi,
)
from modules.data import (
    EPS_Q_ALIASES,
    SALES_Q_ALIASES,
    add_clean_ipo_date_col,
    coalesce_columns,
    fetch_screener_data,
    get_nse_circuit_bands,
)
...
def render_screener_tab():
    ...
    if results_df.empty:
        st.warning(
            "No stocks matched your criteria. Adjust your sidebar filters or switch to another Preset."
        )
    else:
        df = results_df.copy()
        ...
        if en_ipo and ipo_filter_choice != "All Stocks (No IPO Filter)":
            ...
            if ipo_filter_choice == "Recent IPO: Past 1 Month":
                ...
                df = df[
                    (df["IPO_Date_DT"] < now_dt - pd.DateOffset(years=1))
                    | (df["IPO Date"] == "N/A")
                ]
            elif ipo_filter_choice == "Seasoned: Listed > 3 Years Ago":
                df = df[
                    (df["IPO_Date_DT"] < now_dt - pd.DateOffset(years=3))
                    | (df["IPO Date"] == "N/A")
                ]
            elif ipo_filter_choice == "Seasoned: Listed > 5 Years Ago":
                df = df[
                    (df["IPO_Date_DT"] < now_dt - pd.DateOffset(years=5))
                    | (df["IPO Date"] == "N/A")
                ]
        ...
        st.markdown("---")
        with st.expander(
            "📐 Quantitative Pattern Recognition Engine (Dan Zanger / Minervini)", expanded=False
        ):
            st.markdown(
                "Filter candidates via multi-pivot linear regression, apex convergence, and action"
                " trigger timing."
            )

            c_en, c_mode = st.columns([1, 2])
            with c_en:
                en_patterns = st.checkbox(
                    "Enable Pattern Engine", value=st.session_state.get("f_en_patterns", False)
                )
            with c_mode:
                combo_mode = st.selectbox(
                    "Match Logic:",
                    options=[
                        "Match ANY Selected Pattern / Trigger",
                        "Require Action Trigger INSIDE a Base",
                    ],
            ...
            with col_b1:
                pat_flat = st.checkbox(
                    "📐 Flat Base (10+ Bars < 15%)",
                    value=st.session_state.get("f_pat_flat", True),
                    disabled=not en_patterns,
                )
            with col_b2:
                pat_flag = st.checkbox(
                    "🚩 Bull Flag / Pennant",
                    value=st.session_state.get("f_pat_flag", True),
                    disabled=not en_patterns,
                )
            with col_b3:
                pat_asc_tri = st.checkbox(
                    "📐 Ascending Triangle",
                    value=st.session_state.get("f_pat_asc_tri", True),
                    disabled=not en_patterns,
                )
            with col_b4:
                pat_desc_tri = st.checkbox(
                    "🔻 Descending Triangle",
                    value=st.session_state.get("f_pat_desc_tri", False),
                    disabled=not en_patterns,
                )
            with col_b5:
                pat_sym_tri = st.checkbox(
                    "🔷 Symmetrical Triangle",
                    value=st.session_state.get("f_pat_sym_tri", True),
                    disabled=not en_patterns,
                )

            st.caption(
                "⚡ **Action Triggers (Daily Entry Timing — Upper 40% Close + Range < ATR14):**"
            )
            col_t1, col_t2, col_t3, col_t4 = st.columns(4)
            with col_t1:
                pat_inside = st.checkbox(
                    "🎯 Inside Bar (NR14 + Vol Dry)",
                    value=st.session_state.get("f_pat_inside", True),
                    disabled=not en_patterns,
                )
            with col_t2:
                pat_ema21 = st.checkbox(
                    "⚡ 21 EMA Shakeout",
                    value=st.session_state.get("f_pat_ema21", True),
                    disabled=not en_patterns,
                )
            with col_t3:
                pat_ema10 = st.checkbox(
                    "🔥 10 EMA Shakeout",
                    value=st.session_state.get("f_pat_ema10", True),
                    disabled=not en_patterns,
                )
            with col_t4:
                pat_gap_rev = st.checkbox(
                    "🔄 Gap Down Reversal",
                    value=st.session_state.get("f_pat_gap_rev", True),
                    disabled=not en_patterns,
                )
        ...
        if en_patterns and not df.empty and any(pat_config.values()):
            ...
            if df.empty:
                st.warning(
                    "No stocks passed the selected geometric pattern filters. Try turning off"
                    " pattern screening or selecting different patterns."
                )
        ...
        if not df.empty:
            ...
            sec_counts["% of Sector Total"] = sec_counts.apply(
                lambda r: round(
                    (r["Stocks Passed"] / total_sector_counts.get(r["Sector"], 1)) * 100, 1
                ),
                axis=1,
            )
            ...
            ind_counts["% of Industry Total"] = ind_counts.apply(
                lambda r: round(
                    (r["Stocks Passed"] / total_industry_counts.get(r["Basic Industry"], 1)) * 100,
                    1,
                ),
                axis=1,
            )
            ...
            if not active_industries:
                ...
                if ind_chart_state and isinstance(ind_chart_state, dict):
                    ...
                    if pts:
                        extracted = [p.get("label", "") for p in pts if "label" in p]
                        active_industries = [
                            s for s in extracted if s in ind_counts["Basic Industry"].values
                        ]
            ...
            with tab_industry_sum:
                ...
                with c_chart2:
                    fig_ind = px.pie(
                        ind_counts, names="Basic Industry", values="Stocks Passed", hole=0.55
                    )
            ...
            if active_sectors or active_industries:
                ...
                with col_info:
                    st.info(
                        f"🔍 **Active Drilldown:** {' | '.join(filter_labels)} ({len(df_display)}"
                        " Stocks)"
                    )
                with col_reset:
                    if st.button(
                        "🔄 Reset Scan Results (Show All)", type="primary", use_container_width=True
                    ):
            ...
            df_display["Screener_Link"] = (
                "https://www.screener.in/company/" + df_display["name"] + "/consolidated/"
            )
            ...
            fund_badge_map = {
                k: f"{format_v(v.get('verdict', ''))} ({v.get('date', '')})"
            ...
            with col_s1:
                sort_by = st.selectbox(
                    "🔀 Sort Results By:", options=sort_options, index=0, key=f"scan_sort_{rc}_{sc}"
                )
            with col_s2:
                st.write("")
                st.write("")
            ...
            with col_s5:
                cross_metric_exc = st.multiselect(
                    ...
                    help=(
                        "Automatically calculates the Top X% of the selected metrics and"
                        " removes those stocks from your current view to prevent overlap."
                    ),
                )
            ...
            if cross_filter_wls:
                ...
                for f_wl in cross_filter_wls:
                    valid_symbols.update([
                        s.split(":")[-1].strip().upper()
                        for s in st.session_state.watchlists.get(f_wl, [])
                    ])
                ...
                for f_wl in wls_to_check:
                    if f_wl in st.session_state.watchlists:
                        exclude_symbols.update([
                            s.split(":")[-1].strip().upper()
                            for s in st.session_state.watchlists[f_wl]
                        ])
            ...
            if en_top_pct and total_before_slice > 0 and cross_metric_exc:
                ...
                for metric in cross_metric_exc:
                    if metric in df_display.columns:
                        temp_top = df_display.sort_values(
                            by=metric, ascending=False, na_position="last"
                        ).head(num_to_keep)
                ...
                if len(cross_exclude_symbols) > 0:
                    st.info(
                        "🚀 **Momentum Overlap Excluder:** Automatically removed"
                        f" {len(cross_exclude_symbols)} stocks that already appeared in the Top"
                        f" {top_pct_val}% of your selected exclusion metrics."
                    )
            ...
            if sort_by != "Original Scan Order" and sort_by in df_display.columns:
                ...
                df_display = df_display.sort_values(
                    by=temp_col, ascending=sort_asc, na_position="last"
            ...
            if en_top_pct and total_before_slice > 0:
                ...
                results_heading = (
                    f"📋 Scan Results ({len(df_display)} Fresh Stocks Shown — Top {top_pct_val}%"
                    " Quota)"
                )
            else:
            ...
            st.caption(
                "💡 **RS Rating:** IBD-Style 1-99 Percentile Score calculated across 4,000+ listed"
                " Indian equities before filters."
            )
            ...
            with f_col2:
                force_reanalyze_scan = st.checkbox(
                    ...
                    help="If checked, AI will re-fetch data even if a report already exists.",
                )
            with f_col3:
            ...
            if run_batch_scan and len(selected_rows) > 0:
                with st.status(
                    "🧠 Minervini Fundamental AI Analyst — Active Queue", expanded=True
                ) as status_box:
                    ...
                    for idx, sym in enumerate(selected_rows):
                        ...
                        if clean_sym in st.session_state.fundamental_reports and not force_reanalyze_scan:
                            ...
                            status_box.write(
                                f"⚙️ **[{idx + 1}/{len(selected_rows)}] {clean_sym}:** Extracting"
                                " live data & Running AI..."
                            )
            ...
            if len(selected_rows) == 1:
                st.markdown("---")
                active_chart_sym = selected_rows[0]
                clean_ticker = active_chart_sym.split(":")[-1].strip().upper()
                yf_sym = (
                    f"{clean_ticker}.BO"
                    if "BSE" in active_chart_sym.upper()
            ...
            if len(selected_rows) > 0:
                st.subheader(
                    f"📋 Copy Selected Setups to TradingView ({len(selected_rows)} Stocks)"
                )
                st.code(", ".join(selected_rows), language="text")

            filtered_symbols = df_display["TV_Symbol"].tolist()
            st.subheader(
                f"📋 Copy Filtered Scan Results to TradingView ({len(filtered_symbols)} Stocks)"
            )
            ...
            if filtered_symbols:
                ...
                if len(batches) > 1:
                    ...
                    st.caption(
                        ...
                        " **`Backspace`** → **`Ctrl+V`** in your TV watchlist box to hot-swap 30"
                        " stocks at a time!"
                    )
                    batch_labels = []
